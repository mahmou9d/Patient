...
const Success =  ({
  ...
  const { Doctors } = useAppSelector((state: RootState) => state.getdoctors);
   const { appointments:axxx } = useAppSelector(
     (state: RootState) => state.getAppointmentDoctor
   );
  console.log(axxx?.info?.doctor, "axxx");
  const dispatch = useAppDispatch();
    const {
      appointments:appo,
      loading: allLoading,
  ...
  useEffect(() => {
    ...
    dispatch(getAppointmentDoctor(appointmentId as string));
        ...
        <div className="success-img">
          ...
          <section className="request-details">
            ...
            <div className="flex items-center gap-3">
              <Image
                src={
                  axxx?.info?.doctor?.img_url! || "/assets/images/dr-peter.png"
                }
                alt="doctor"
                width={100}
                height={100}
              ...
              <p className="whitespace-nowrap">
                Dr.{" "}
                {`${axxx?.info?.doctor?.first_name} ${axxx?.info?.doctor?.last_name}`}
              </p>
            </div>
            <div className="flex gap-2">
              ...
              <p>
                ...
                {
                  formatDateTime(
                    axxx?.info?.expected_appointment_date ?? new Date()
                  ).dateTime
                }
              </p>
            </div>
